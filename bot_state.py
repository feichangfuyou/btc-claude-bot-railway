...
from trade_memory import record_market_snapshot, record_trade_memory, run_learning_cycle
from trading_presets import get_min_rr, get_sl_tp_for_regime
...
class BotState:
    ...
    def can_trade(self, symbol: str = None) -> tuple[bool, str]:
        ...
        cooldown_mult = 1 + min(self.consecutive_losses, 2)
        ...
        if age < effective_cooldown:
            ...
            return (
                False,
                f"trade cooldown: {remaining}s remaining (cooldown={effective_cooldown}s after {self.consecutive_losses} losses)",
            )
    ...
    def _track_consecutive(self, net: float):
        if net < 0:
            ...
            # Winning trade breaks the streak → clear circuit breaker so bot can trade again
            if self.loss_breaker_active:
                self.loss_breaker_active = False
    ...
    def _handle_open_trade(self, action: str, decision: dict):
        ...
        if qual == "choppy" and confidence < 0.70:
            self.last_ai_block_reason = (
                f"{ai_msg} — rejected: price action choppy (conf {confidence * 100:.0f}% < 70%)"
            )
            self.add_log(self.last_ai_block_reason, "warning")
            return
        ...
        if ENABLE_FUTURES and TRADE_MODE in ("futures", "both") and not self.get_position_for_symbol(symbol):
            ...
            if n_futures < MAX_FUTURES_POSITIONS:
                from futures_executor import execute_futures_live, execute_futures_paper
        ...
        if is_onchain:
            from onchain_executor import execute_onchain
        ...
        self.add_log(
            f"{emoji} {action.upper()} {symbol} @ ${entry:,.2f} | "
            f"TP ${tp:,.2f} | SL ${sl:,.2f} | "
    ...
    def snapshot(self) -> dict:
        ...
        try:
            from trade_memory import build_memory_briefing
        ...
        try:
            from claude_ai import get_cost_tracker
