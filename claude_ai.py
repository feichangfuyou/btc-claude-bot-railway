...
from trade_memory import build_memory_briefing, get_pattern_verdict
from trading_presets import get_preset
...
def get_cost_tracker() -> dict:
    ...
    t = _api_cost_tracker
    total = t["total_scout_cost"] + t["total_trade_cost"]
    return {
...
def _extract_json(raw: str) -> dict:
    ...
    truncated responses (Unterminated string). Uses raw_decode to parse exactly one object."""
    raw = raw.strip()

    # 1. Strip markdown fences so we work on plain JSON
    md_match = re.search(r"```(?:json)?\s*(\{)", raw, re.DOTALL | re.IGNORECASE)
    if md_match:
        start = md_match.end(1) - 1  # index of opening {
    else:
        start = raw.find("{")
    if start == -1:
        raise json.JSONDecodeError("No JSON object found", raw, 0)

    # 2. raw_decode parses exactly ONE JSON value — fixes "Extra data" and ignores trailing content
...
def _build_scout_snapshot(bot, coins_snapshot: dict) -> str:
    ...
    return (
        ...
        "Return EXACTLY ONE JSON object (not multiple):"
    )
...
async def call_claude(bot, broadcast_price_fn, skip_scout: bool = False):
    ...
    if age > PRICE_MAX_AGE_SEC:
        bot.add_log(
            f"Prices are {age:.0f}s stale (>{PRICE_MAX_AGE_SEC}s) — skipping",
            "warning",
        )
        return
    ...
    async with _claude_lock:
        ...
        progress_pct = round(progress / max(PROFIT_TO_TARGET, 1) * 100, 1)
        ...
        if losing_streak >= 3:
            ...
            anti_overtrade["message"] = (
                f"LOSING STREAK ({losing_streak}): Raise bar—5+ signals, 55%+ confidence."
            )
        ...
        snap = {
            ...
            "account": {
                **bot.account,
                "can_trade": ok,
                "block_reason": block_reason,
                "profit_target": TARGET_BALANCE,
                "profit_to_make": PROFIT_TO_TARGET,
                "progress_pct": progress_pct,
                "remaining_to_target": round(PROFIT_TO_TARGET - progress, 2),
            },
            "cost_info": {
            ...
            "mission": (
                f"COMPOUND ${balance:.0f} → ${TARGET_BALANCE:.0f} (need +${PROFIT_TO_TARGET:.0f} profit). "
                f"Currently +${progress:.2f} ({progress_pct}%). "
        ...
        try:
            ...
            if action in ("buy", "sell"):
                from config import REQUIRE_TRADE_APPROVAL
