...
def _generate_cdp_jwt(method: str, path: str) -> str:
    ...
    headers = {"kid": CDP_KEY_NAME, "typ": "JWT"}

    try:
        # Note: private key must be in PEM format
        token = jwt.encode(payload, CDP_PRIVATE_KEY, algorithm="ES256", headers=headers)
        return token
...
def create_coinbase_charge(
    ...
    try:
        ...
        response = requests.post(f"https://business.coinbase.com{path}", json=payload, headers=headers, timeout=10)
