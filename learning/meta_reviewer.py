...
def get_meta_guidance() -> str:
    ...
    if os.path.exists(MONTHLY_GUIDANCE_PATH):
        with open(MONTHLY_GUIDANCE_PATH, "r") as f:
    ...
    if os.path.exists(WEEKLY_GUIDANCE_PATH):
        with open(WEEKLY_GUIDANCE_PATH, "r") as f:
            content = f.read().strip()
            if content:
    ...
    if os.path.exists(DAILY_GUIDANCE_PATH):
        with open(DAILY_GUIDANCE_PATH, "r") as f:
            content = f.read().strip()
            if content:
...
def _get_review_stats(days: int) -> dict:
    ...
    try:
        ...
        sessions = conn.execute(
            "SELECT * FROM session_stats WHERE date >= ? ORDER BY date ASC",
            (since_date,)
        ).fetchall()
...
GUIDANCE_PATHS = {
    "daily": DAILY_GUIDANCE_PATH,
    "weekly": WEEKLY_GUIDANCE_PATH,
    "monthly": MONTHLY_GUIDANCE_PATH
}
