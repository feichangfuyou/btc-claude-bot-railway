"""Tests for Kraken Executor — live/paper spot order execution on Kraken."""
...
@pytest.fixture
def decision():
    return {"confidence": 0.80, "patterns_detected": ["momentum"]}


EXEC_MODULE = "executors.kraken_executor"


class TestExecuteKraken:
    @pytest.mark.asyncio
    async def test_buy_success(self, mock_bot, decision):
        with (
            patch(f"{EXEC_MODULE}.resolve_exchange_keys", return_value=("key", "secret")),
            patch(f"{EXEC_MODULE}.ENABLE_KRAKEN", True),
            patch(f"{EXEC_MODULE}.is_configured", return_value=True),
            patch(
                f"{EXEC_MODULE}.add_market_order_by_quote",
                new_callable=AsyncMock,
                return_value="txid-buy-1234567890123",
            ),
        ):
            from executors.kraken_executor import execute_kraken

            await execute_kraken(mock_bot, "buy", "BTC", 95000, 97000, 93000, 0.01, 950, decision)
        ...
        assert pos["order_id"] == "txid-buy-1234567890123"
    ...
    async def test_sell_success(self, mock_bot, decision):
        with (
            ...
            patch(f"{EXEC_MODULE}.ENABLE_KRAKEN", True),
            patch(f"{EXEC_MODULE}.is_configured", return_value=True),
            patch(f"{EXEC_MODULE}.add_market_order", new_callable=AsyncMock, return_value="txid-sell-123456789012"),
        ):
            from executors.kraken_executor import execute_kraken

            await execute_kraken(mock_bot, "sell", "BTC", 95000, 93000, 97000, 0.01, 950, decision)
    ...
    async def test_fallback_to_paper_when_not_configured(self, mock_bot, decision):
        with (
            ...
            patch(f"{EXEC_MODULE}.ENABLE_KRAKEN", False),
        ...
        mock_bot.add_log.assert_any_call("⚠ Kraken disabled or not configured — falling back to paper", "warning")
    ...
    async def test_fallback_to_paper_on_order_failure(self, mock_bot, decision):
        with (
            ...
            patch(f"{EXEC_MODULE}.ENABLE_KRAKEN", True),
            patch(f"{EXEC_MODULE}.is_configured", return_value=True),
            patch(f"{EXEC_MODULE}.add_market_order_by_quote", new_callable=AsyncMock, return_value=None),
        ):
            from executors.kraken_executor import execute_kraken
    ...
    async def test_fallback_to_paper_on_exception(self, mock_bot, decision):
        with (
            ...
            patch(
                f"{EXEC_MODULE}.add_market_order_by_quote",
                new_callable=AsyncMock,
                side_effect=ConnectionError("Kraken unreachable"),
            ),
        ):
            from executors.kraken_executor import execute_kraken
...
class TestCloseKraken:
    ...
    async def test_close_long_profit(self, mock_bot):
        ...
        with (
            ...
            patch(f"{EXEC_MODULE}.add_market_order", new_callable=AsyncMock, return_value="close-txid-1234"),
            ...
            await close_kraken(mock_bot, pos, "TP HIT")
    ...
    async def test_close_long_loss(self, mock_bot):
        ...
        with (
            patch(f"{EXEC_MODULE}.resolve_exchange_keys", return_value=("k", "s")),
            patch(f"{EXEC_MODULE}.add_market_order", new_callable=AsyncMock, return_value="close-txid-5678"),
            patch(f"{EXEC_MODULE}.db_save_trade"),
            patch(f"{EXEC_MODULE}.record_trade_memory"),
            patch(f"{EXEC_MODULE}.run_learning_cycle") as mock_learn,
            patch(f"{EXEC_MODULE}.send_notification", new_callable=AsyncMock),
            ...
            await close_kraken(mock_bot, pos, "SL HIT")
    ...
    async def test_close_short_profit(self, mock_bot):
        ...
        with (
            patch(f"{EXEC_MODULE}.resolve_exchange_keys", return_value=("k", "s")),
            patch(f"{EXEC_MODULE}.add_market_order", new_callable=AsyncMock, return_value="close-txid-9012"),
            patch(f"{EXEC_MODULE}.db_save_trade"),
            patch(f"{EXEC_MODULE}.record_trade_memory"),
            patch(f"{EXEC_MODULE}.run_learning_cycle") as mock_learn,
            patch(f"{EXEC_MODULE}.send_notification", new_callable=AsyncMock),
    ...
    async def test_close_failure_falls_back(self, mock_bot):
        ...
        with (
            patch(f"{EXEC_MODULE}.resolve_exchange_keys", return_value=("k", "s")),
            patch(f"{EXEC_MODULE}.add_market_order", new_callable=AsyncMock, return_value=None),
            patch(f"{EXEC_MODULE}.send_notification", new_callable=AsyncMock),
        ):
            from executors.kraken_executor import close_kraken

            await close_kraken(mock_bot, pos, "CLOSE ATTEMPT")

        mock_bot.finalize_paper_close.assert_called_once_with(pos, 95000.0, "CLOSE ATTEMPT", exchange="kraken")
